...
const featuredCategories = [
  { 
    ...
    description: 'Access rare, artisan, and highly sought-after niche fragrances.',
    image: 'https://images.unsplash.com/photo-1610461888750-10bfc601b874?q=80&w=1000&auto=format&fit=crop', 
    link: '/catalog',
    color: 'emerald'
  },
  { 
    name: 'Global Dropshipping', 
    description: 'We fulfill orders directly to your customers in elegant, unbranded packaging.',
    image: 'https://images.unsplash.com/photo-1594824419992-ea9b699e19d7?q=80&w=1000&auto=format&fit=crop', 
    link: '/catalog?category=logistics',
    color: 'rose'
  },
  { 
    name: 'Designer Portfolio', 
    description: 'Stock the world\'s most recognized luxury designer scents at wholesale prices.',
    image: 'https://images.unsplash.com/photo-1594035910387-fea477242610?q=80&w=1000&auto=format&fit=crop', 
    link: '/catalog?category=sync',
    color: 'blue'
  }
];
    ...
    <div className="bg-slate-50 overflow-x-hidden">
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-dark text-white pt-20">
        ...
        <div className="relative z-10 text-center px-4 max-w-5xl mx-auto">
          ...
          >
             <Link to="/catalog" className="group relative px-10 py-5 rounded-sm bg-primary text-white text-[11px] uppercase tracking-[0.2em] font-bold shadow-[0_0_20px_rgba(247,27,99,0.2)] hover:shadow-[0_0_30px_rgba(247,27,99,0.4)] transition-all overflow-hidden border border-primary">
                <span className="relative z-10">Browse Catalog</span>
                <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-1000" />
             </Link>
             <Link to="/contact" className="px-10 py-5 rounded-sm bg-transparent text-white text-[11px] uppercase tracking-[0.2em] font-bold border border-slate-600 hover:bg-white hover:text-dark transition-all backdrop-blur-sm">
                Apply for Access
             </Link>
          </motion.div>
          ...
          <motion.div 
             ...
             className="mt-28 pt-10 border-t border-white/5 flex flex-wrap justify-center gap-x-12 gap-y-6 grayscale opacity-40 hover:grayscale-0 hover:opacity-80 transition-all"
          >
            {partners.map(brand => (
              <span key={brand} className="text-sm font-sans font-black tracking-widest uppercase">{brand}</span>
            ))}
          </motion.div>
        </div>
      </section>
      ...
      <section className="relative z-20 py-24 bg-white border-y border-slate-100">
        <div className="max-w-7xl mx-auto px-4 grid grid-cols-2 md:grid-cols-4 gap-12 text-center">
           <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={itemVariants}>
              <span className="block text-4xl font-sans font-bold tracking-tighter text-dark mb-2">1,500+</span>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          >
            ...
            <motion.div variants={itemVariants} className="group p-10 rounded-2xl bg-white border border-slate-200 shadow-sm hover:shadow-xl transition-all duration-500 hover:-translate-y-2">
               ...
               <Link to="/download" className="text-[10px] uppercase tracking-[0.2em] font-black text-dark flex items-center gap-2 group-hover:text-primary transition-colors">
            ...
            <motion.div variants={itemVariants} className="group p-10 rounded-2xl bg-white border border-slate-200 shadow-sm hover:shadow-xl transition-all duration-500 hover:-translate-y-2">
               ...
               <Link to="/about" className="text-[10px] uppercase tracking-[0.2em] font-black text-dark flex items-center gap-2 group-hover:text-primary transition-colors">
            ...
            <motion.div variants={itemVariants} className="group p-10 rounded-2xl bg-white border border-slate-200 shadow-sm hover:shadow-xl transition-all duration-500 hover:-translate-y-2">
               ...
               <Link to="/register" className="text-[10px] uppercase tracking-[0.2em] font-black text-dark flex items-center gap-2 group-hover:text-primary transition-colors">
      ...
      {/* Featured Categories (Updated style) */}
      <section className="py-32 bg-white">
        <div className="max-w-7xl mx-auto px-4">
           <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-8 text-left">
              <div>
                <span className="text-primary text-[10px] uppercase tracking-[0.4em] font-black mb-4 block">Product Access</span>
                <h2 className="text-4xl md:text-5xl font-sans font-bold tracking-tighter text-dark ">Select Categories</h2>
              </div>
              <Link to="/catalog" className="px-8 py-3 bg-slate-100 text-dark text-[11px] uppercase tracking-widest font-black rounded-sm hover:bg-dark hover:text-white transition-all">
                Full Catalog &rarr;
              </Link>
           </div>
           ...
           <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
             {featuredCategories.map((cat, idx) => (
               ...
               >
                  ...
                  <div className="p-8">
                     <h3 className="text-2xl font-bold text-dark mb-3">{cat.name}</h3>
                     <p className="text-slate-500 text-sm leading-relaxed mb-8">{cat.description}</p>
                     <Link to={cat.link} className="inline-flex items-center gap-2 text-primary text-[10px] uppercase tracking-[0.2em] font-black group-hover:translate-x-2 transition-transform">
        ...
        <div className="max-w-4xl mx-auto text-center relative z-10">
          ...
          >
             ...
             <div className="flex flex-col sm:flex-row justify-center items-center gap-6">
                <Link to="/register" className="w-full sm:w-auto px-12 py-5 bg-primary text-white text-[11px] uppercase tracking-[0.2em] font-bold rounded-sm shadow-xl shadow-primary/30 hover:scale-105 active:scale-95 transition-all">
                  Join the Network
                </Link>
                <Link to="/contact" className="w-full sm:w-auto px-12 py-5 bg-dark text-white text-[11px] uppercase tracking-[0.2em] font-bold rounded-sm hover:bg-slate-800 transition-all">
                  Talk to Sourcing
                </Link>
             </div>
          </motion.div>
